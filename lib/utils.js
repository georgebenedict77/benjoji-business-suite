...
function requireUsername(value) {
  ...
  if (!/^[a-zA-Z0-9._-]{3,30}$/.test(username)) {
    throw createHttpError("Username must be 3-30 characters and use letters, numbers, dots, dashes, or underscores.");
  }
  return username;
}
...
function requirePin(value) {
  ...
  if (!/^\d{6}$/.test(pin)) {
    throw createHttpError("Access PIN must be exactly 6 digits.");
  }
  return pin;
}
...
function requirePositiveNumber(value, fieldName) {
  ...
  if (!Number.isFinite(amount) || amount <= 0) {
    throw createHttpError(`${fieldName} must be greater than zero.`);
  }
  return amount;
}
...
function requirePositiveInteger(value, fieldName) {
  ...
  if (!Number.isInteger(amount) || amount <= 0) {
    throw createHttpError(`${fieldName} must be a whole number greater than zero.`);
  }
  return amount;
}
...
function normalizeRole(value) {
  ...
  if (!["OWNER", "STAFF"].includes(role)) {
    throw createHttpError("Role must be OWNER or STAFF.");
  }
  return role;
}
