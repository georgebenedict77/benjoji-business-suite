...
function verifyUserAccess(options) {
  ...
  if (!user) {
    throw createHttpError(authMode === "pin" ? "Invalid workspace, username, or PIN." : "Invalid workspace, username, or password.", 401);
  }
...
function shouldUseSecureCookies(req) {
  ...
  const trustProxy = process.env.BENJOJI_TRUST_PROXY === "true" || process.env.RENDER === "true";
  if (!trustProxy) {
    return false;
  }
...
function completeSecondFactorLogin(req, res, { challengeId, pin, workspaceKey }) {
  ...
  createSession(req, res, safeWorkspaceKey, user.id);
