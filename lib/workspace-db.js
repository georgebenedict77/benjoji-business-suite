...
const APP_DIR_NAME = "Benjoji Business Suite";
const LEGACY_APP_DIR_NAMES = ["BENJOJI Payment Handling"];
...
function resolveDataDirRoot() {
  ...
  const legacyCandidates = LEGACY_APP_DIR_NAMES.flatMap((name) => [
    process.env.LOCALAPPDATA ? path.join(process.env.LOCALAPPDATA, name) : null,
    process.env.APPDATA ? path.join(process.env.APPDATA, name) : null,
  ]).filter(Boolean);
  const projectCandidate = path.resolve(__dirname, "..", "data");

  for (const candidate of [...legacyCandidates, ...preferredCandidates, projectCandidate]) {
    if (containsPersistentData(candidate) && isWritableDir(candidate)) {
      return candidate;
    }
  }

  for (const candidate of [...preferredCandidates, projectCandidate]) {
...
function containsPersistentData(candidate) {
  if (!candidate || !fs.existsSync(candidate)) {
    return false;
  }
  return fs.existsSync(path.join(candidate, "workspaces")) || fs.existsSync(path.join(candidate, DB_FILE_NAME));
}
...
function getWorkspaceSummary(workspaceKey) {
  ...
  if (hasDatabase) {
    try {
      ...
      logoDataUrl = businessProfile.logoDataUrl || logoDataUrl;
