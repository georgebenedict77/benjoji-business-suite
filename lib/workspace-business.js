...
function addOrStockInProduct(workspaceKey, { name, productCode, unitPrice, quantity, authorizedBy }) {
  ...
  recordAutomaticBackup(workspaceKey, "stock-in", safeAuthorizedBy);
}
...
function adjustProductStock(workspaceKey, { productId, quantity, actionType, authorizedBy }) {
  ...
  recordAutomaticBackup(workspaceKey, `manual-${safeActionType.toLowerCase()}`, safeAuthorizedBy);
}
...
function createSale(workspaceKey, { customerName, phoneNumber, processedBy, items, payments }) {
  ...
  const loadedPayments = buildPayments(workspaceKey, payments || []);
  ...
  recordAutomaticBackup(workspaceKey, "sale-completed", safeProcessedBy);
...
function createDebtPayment(workspaceKey, { customerName, payments }) {
  ...
  recordAutomaticBackup(workspaceKey, "debt-payment", safeCustomerName);
...
function reportRange(kind, date) {
  const safeKind = requireText(kind, "Report type").toLowerCase();
  if (safeKind === "weekly") {
    return {
