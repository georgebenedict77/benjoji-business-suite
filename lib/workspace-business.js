...
function buildReport(workspaceKey, kind, date) {
  ...
  for (const row of salesRows) {
    const sale = mapSale(workspaceKey, row);
    sales.push(sale);
  ...
  for (const row of creditRows) {
    ...
    credits.push(credit);
    totalCredit += credit.originalAmount;
    outstandingDebt += credit.amountOwed;
  }
  ...
  for (const row of stockRows) {
    if (row.action_type === "STOCK_IN") {
      stockIn += 1;
    }
    if (row.action_type === "STOCK_OUT") {
      stockOut += 1;
    }
  ...
  const sortedProducts = [...productTotals.entries()].sort((left, right) => right[1] - left[1]);
  for (const [productName, quantity] of sortedProducts.slice(0, 5)) {
    mostSoldProducts.push({ productName, quantity });
  }
  ...
  return {
    ...
    outstandingDebt: roundAmount(outstandingDebt),
    paymentBreakdown,
    mostSoldProducts,
    stockMovement: {
      stockIn,
...
function buildPayments(workspaceKey, inputPayments) {
  ...
  for (const payment of inputPayments) {
    ...
    const paymentMetaInput = {
      ...route,
      ...payment,
    };
    paymentMetaInput.targetNumber = payment.targetNumber || route.targetNumber;
    const paymentMeta = sanitizePaymentMeta(paymentMethod, paymentMetaInput);
...
function mapSale(workspaceKey, row) {
  ...
  for (const payment of paymentRows) {
    const paymentMeta = parsePaymentMeta(payment.meta_json);
    payments.push({
      ...
      paymentMeta,
      detailSummary: summarizePaymentDetail(payment.payment_method, paymentMeta),
      messages: buildPaymentMessages(payment.payment_method, payment.amount, paymentMeta),
    });
  }
  ...
  return {
    ...
    subtotalAmount: roundAmount(subtotalAmount),
