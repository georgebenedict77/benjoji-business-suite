...
function restoreBackupSnapshot(workspaceKey, fileName, actor = "System") {
  const { fileName: safeFileName, snapshot } = getBackupSnapshot(workspaceKey, fileName);
  ...
  const emergencyBackup = createBackupSnapshot(workspaceKey, `pre-restore:${safeFileName}`, actor);
...
function resolveBackupSnapshotPath(workspaceKey, fileName) {
  ...
  const backupPath = path.resolve(getWorkspaceBackupDir(workspaceKey), safeName);
